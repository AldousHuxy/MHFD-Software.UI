...
const culvertSchema = z.object({
    culvertId: z.string().min(1, 'Culvert ID is required').max(50, 'Culvert ID must be less than 50 characters')
});

type CulvertFormData = z.infer<typeof culvertSchema>;

type NewCulvertModalProps = {
    isOpen: boolean;
    onClose: () => void;
    onSubmit: (data: CulvertFormData, culvertType: CulvertType) => void;
}

export const NewCulvertModal = ({ isOpen, onClose, onSubmit }: NewCulvertModalProps) => {
    const [culvertType, setCulvertType] = useState<CulvertType>('pipe');
    const { register, handleSubmit, formState: { errors }, reset } = useForm<CulvertFormData>({
        resolver: zodResolver(culvertSchema)
    });

    const handleFormSubmit: SubmitHandler<CulvertFormData> = (data) => {
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black/40" style={{ zIndex: 9999 }} onClick={handleClose}>
            <div className="bg-white rounded-2xl shadow-[0_20px_60px_-15px_rgba(0,0,0,0.3)] p-8 w-full max-w-md relative transform transition-all" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="flex items-center justify-between mb-6 mt-2">
                    <h2 className="text-2xl font-bold text-mhfd-dark-blue">New Culvert</h2>
                ...
                <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-4">
                    <div>
                        <label htmlFor="culvertId" className="block text-sm font-semibold text-gray-700 mb-2">
                            {culvertType === 'pipe' ? 'Pipe ID' : 'Box ID'}
                        </label>
                        <input
                            id="culvertId"
                            type="text"
                            ...
                            placeholder={`Enter ${culvertType === 'pipe' ? 'pipe' : 'box'} ID`}
                        />
                        {errors.culvertId && (
